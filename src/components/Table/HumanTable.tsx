...
interface EnhancedTableProps {
  title: string;
  numSelected: number;
  onRequestSort: (event: React.MouseEvent<unknown>, property: keyof humanData) => void;
...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler =
    (property: keyof humanData) => (event: React.MouseEvent<unknown>) => {
...
export default function EnhancedTable(props:EnhancedTable) {
  ...
  const visibleRows =React.useMemo(
    () =>
      stableSort(rows, getComparator(order, orderBy as keyof humanData)).slice(
        page * rowsPerPage,
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="center">{Number(row.bloodGlucose).toFixed(2)}</TableCell>
                    <TableCell align="center">{Number(row.HbA1c).toFixed(1)}</TableCell>
                    <TableCell align="center">{Number(row.BMI).toFixed(1)}</TableCell>
                  </TableRow>
                );
              })}
              {emptyRows > 0 && (
